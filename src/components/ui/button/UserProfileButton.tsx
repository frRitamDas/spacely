...
import { addToast, Avatar, Button, Dropdown, DropdownItem, DropdownMenu, DropdownTrigger, Popover, PopoverContent, PopoverTrigger, Spinner } from "@heroui/react";
...
const UserProfileButton: React.FC = () => {
  ...
  return (
    <Popover placement="bottom-end" offset={10} isOpen={settingsOpen} onOpenChange={setSettingsOpen}>
      <PopoverTrigger>{profileButton}</PopoverTrigger>
      <PopoverContent className="overflow-visible bg-transparent p-0 shadow-none">
        {settingsOpen ? (
          <SettingsPopover onBack={() => setSettingsOpen(false)} />
        ) : (
          <div className="w-64 overflow-hidden rounded-[22px] border border-white/10 bg-[#101114]/95 p-2 text-white shadow-[0_28px_90px_rgba(0,0,0,.7)] backdrop-blur-3xl">
            ...
            <Dropdown showArrow={false} className="hidden"><DropdownTrigger><span /></DropdownTrigger><DropdownMenu aria-label="hidden" /></Dropdown>
