...
import { addToast, Avatar, Button, Dropdown, DropdownItem, DropdownMenu, DropdownTrigger, Spinner } from "@heroui/react";
...
const UserProfileButton: React.FC = () => {
  ...
  const ITEMS: DropdownItemProps[] = useMemo(() => [
    {
      ...
      onClick: async () => {
        ...
        if (!success) return setLogout(false);
  ...
  return (
    <Dropdown showArrow closeOnSelect={false} className="w-10">
      ...
      <DropdownMenu aria-label="User profile dropdown" variant="flat" disabledKeys={logout ? ITEMS.map((i) => i.label) : undefined}>
        {ITEMS.map(({ label, icon, ...props }) => (
          <DropdownItem key={label} startContent={icon} {...props}>{label}</DropdownItem>
        ))}
      </DropdownMenu>
    </Dropdown>
  );
