...
const PersonalizedRails = dynamic(() => import("@/components/sections/Home/PersonalizedRails"));
const InfiniteCategories = dynamic(() => import("@/components/sections/Home/InfiniteCategories"));

const HomePage: NextPage = () => {
  const settings = useSpacelySettings();
  return <div className="flex flex-col gap-10 pb-16 md:gap-16"><Hero /><div className="flex flex-col gap-12 md:gap-16">{settings.continueWatching && !settings.pauseWatchHistory && <ContinueWatching />}<PersonalizedRails /><HomePageList /><InfiniteCategories /></div></div>;
};
